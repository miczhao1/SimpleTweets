...
class TwitterClient(context: Context) : OAuthBaseClient(
    ...
    companion object {
        val REST_API_INSTANCE = TwitterApi.instance() // Twitter API

        const val REST_URL = "https://api.twitter.com/1.1" // specify Twitter Resource URL (prepends)
    ...
    fun getHomeTimeline(handler: JsonHttpResponseHandler) {
        val apiUrl =
            getApiUrl("statuses/home_timeline.json") // from Resource URL

        // Can specify query string params directly or through RequestParams.
        val params = RequestParams()
        params.put("count", "25") // Parameters
        params.put("since_id", "1")
        client.get(apiUrl, params, handler) // reads data from Twitter
    }

    fun publishTweet(tweetContent: String, handler: JsonHttpResponseHandler) {
        val apiUrl =
            getApiUrl("statuses/update.json") // from Resource URL

        // Can specify query string params directly or through RequestParams.
        val params = RequestParams()
        params.put("status", tweetContent) // required param
        client.post(apiUrl, params, "", handler) // writes data from Twitter
    }
